...
// Copyright (c) 2013 The DHcoin Developers
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "DHcoin server has now stopped running!";
}
...
// DHcoin: Return average network hashes per second based on last number of blocks.
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid DHcoin address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid DHcoin address:")+s.name_);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; DHcoin server stopping, restart to run with encrypted wallet";
}
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "DHcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "DHcoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        result.push_back(Pair("target",   HexStr(BEGIN(hashTarget), END(hashTarget))));
        result.push_back(Pair("algorithm", "scrypt:1024,1,1"));  // DHcoin: specify that we should use the scrypt algorithm
        return result;
    }
    else
    {
...
//
// JSON-RPC protocol.  DHcoin speaks version 1.0 for maximum compatibility,
