...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    int64 nSubsidy = 0;
    if(nHeight<2){
    	nSubsidy = 1000000000000 * COIN;
    }
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10; // DHcoin: every 20 minutes = 20 * 60
static const int64 nTargetSpacing = 1; // DHcoin: 60 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // DHcoin difficulty adjustment protocol switch
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal dhcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DHcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Nov 19, 2015, 11:40 a.m. EDT: The ALMA observatory located in Chile’s Atacama Desert is to join with observatories from all over the world to create a telescope that can identify objects as small as 10 cm on the surface of the moon.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1448280520;//1369199888
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 11277777;//11288888
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // DHcoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
...
void static ThreadDHcoinMiner(void* parg);
...
void static DHcoinMiner(CWallet *pwallet)
{
    ...
    while (fGenerateBitcoins)
    {
        ...
        printf("Running DHcoinMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadDHcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadDHcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadDHcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadDHcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d DHcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadDHcoinMiner, pwallet))
